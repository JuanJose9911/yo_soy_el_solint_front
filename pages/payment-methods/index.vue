<template>
  <div>
    <app-button @click="modal = true">Agregar método de pago</app-button>
    ...
    <app-modal
      v-model="modalEditar"
      :titulo="tituloEditar"
      :textBtn="textBtnEditar"
      @metodoBoton="editPaymentMethod()"
    >
      <app-input
    ...
    <app-modal
      ...
      :textBtn="textBtnEliminar"
      @metodoBoton="removePaymentMethods()"
    >
      <label>¿Seguro que desea eliminar este elemento?</label>
    </app-modal>
  </div>
</template>
...
export default {
  ...
  data: () => ({
    ...
    tituloEliminar: "Eliminar Método",
    tituloEditar: "Editar Método",
  ...
  methods: {
    async getPaymentMethods() {
      this.name = ""
      const response = await this.$axios.get("/api/payment-methods")
      this.items = response.data
    },
    async addPaymentMethods() {
      try {
        const response = await this.$axios.post("/api/payment-methods", {
          name: this.name,
          status: this.status,
        })
        if (response.data) {
    ...
    async removePaymentMethods() {
      ...
      await this.$axios.delete(`api/payment-methods/${id}`)
      await this.getPaymentMethods()
    ...
    async editPaymentMethod() {
      ...
      await this.$axios.put(`api/payment-methods/${id}`, {
